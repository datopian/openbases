# An example environment. Copy this directory, rename it, and fill in
# terraform.tfvars.
#
# It is a copy of envs/staging with our values removed, because the alternative
# -- reading somebody else's environment and guessing which parts are theirs --
# is how an outside deployment ends up half-configured with our hostnames in it.
#
# There is no terraform.tfvars here, only terraform.tfvars.example, so `tofu
# apply` in this directory refuses until somebody has made the file on purpose.
#
module "environment" {
  ...
  # An execution node is where agents run. Keep it separate from the control
  # node: ADR-0002's isolation model is the reason the two are not one box.
  ...
  #
  #   $ curl -X POST https://<your-team>.cloudflareaccess.com/cdn-cgi/access/oauth/registration   #       -d '{"redirect_uris":["https://example.invalid/never-allowed"],...}'

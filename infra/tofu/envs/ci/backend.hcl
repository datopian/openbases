...
# The EXISTING state bucket with a distinct key, not a new bucket.
#
# State is not the sensitive boundary here — the Hetzner project is, and that is
# already separate. Creating a second bucket would mean another apply and
# another R2 token scope for no security gain, and wg-ohk is a fresh reminder of
# what a mis-scoped R2 token costs.

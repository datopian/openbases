...
| Google API scopes, Pub/Sub resources, allow-listed sources, subscription renewal evidence | **complete** | [`2026-08-31-workspace-subscriptions-staging.json`](evidence/2026-08-31-workspace-subscriptions-staging.json). Project `datopian-workgraph-events`, two read-only scopes granted to client ID 102065960434834149554, topic and OIDC push subscription applied. Five allow-listed sources — three shared drives and two Meet spaces — each with one active subscription, renewed hourly by `workgraph-workspace.timer` inside a 24-hour window, and alerted on by `platform_workspace_subscription_lapsed` if a renewal stops. **All seven acceptance criteria pass**, the last of them on a real meeting: the CDT kick-off was registered at 08:29 UTC on 2026-09-01 and held that morning, and `transcript.v2.ended` and `conference.v2.ended` arrived at 12:40 and 12:48 attributed to the source — discovered, not announced. Drive detection proven by 43 receipts, most of them organic activity rather than probes. Two IAM bindings on the push identity are documented prerequisites rather than code, because OpenTofu authenticates as that same service account; both are in the evidence file with the command that detects them. Remaining deviation: the delegation subject is a person's address rather than a dedicated account (`wg-map`). |

...
sql() { on "sudo -u postgres psql -X -q -At -d workgraph -c \"$1\""; }

reconcile() { on "systemctl start workgraph-workspace.service; journalctl -u workgraph-workspace.service -n 40 --no-pager --since '1 min ago'"; }
...
INTERVAL=$(on "systemctl show workgraph-workspace.timer -p TimersMonotonic --value" | tr -d '\r')
...
NOT_ACTIVE=$(sql "SELECT count(*) FROM system_event_sources() WHERE enabled AND state <> 'active';" | tr -d '\r')
...
# inside it.
BEFORE=$(sql "SELECT google_name FROM event_subscriptions s JOIN event_sources src ON src.id = s.source_id WHERE src.kind='drive' AND s.state='active' ORDER BY src.display_name LIMIT 1;" | tr -d '\r')
if [ -z "$BEFORE" ]; then
  ...
  reconcile | tail -12
  AFTER=$(sql "SELECT google_name FROM event_subscriptions WHERE source_id = (SELECT source_id FROM event_subscriptions WHERE google_name = '$BEFORE');" | tr -d '\r')
  EXPIRY=$(sql "SELECT expires_at FROM event_subscriptions WHERE google_name = '$BEFORE';" | tr -d '\r')
...
CALLS_BEFORE=$(sql "SELECT count(*) FROM event_subscriptions;" | tr -d '\r')
APPLIED=$(reconcile | grep -c '"msg":"applied"' || true)
CALLS_AFTER=$(sql "SELECT count(*) FROM event_subscriptions;" | tr -d '\r')
...
VICTIM=$(sql "SELECT id FROM event_sources WHERE kind='drive' AND enabled ORDER BY display_name DESC LIMIT 1;" | tr -d '\r')
VICTIM_NAME=$(sql "SELECT display_name FROM event_sources WHERE id='$VICTIM';" | tr -d '\r')
echo "  disabling: $VICTIM_NAME"
sql "UPDATE event_sources SET enabled = false WHERE id = '$VICTIM';"
reconcile | tail -6
STATE=$(sql "SELECT state FROM event_subscriptions WHERE source_id = '$VICTIM';" | tr -d '\r')
...
ORPHANED=$(sql "SELECT count(*) FROM event_receipts WHERE message_id LIKE 'acceptance-probe-%' AND source_id IS NULL;" | tr -d '\r')
...
on "$BIN -summary -since 168h" || true

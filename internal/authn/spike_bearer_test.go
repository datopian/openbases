...
// spikeToken returns the single token this spike accepts, composed at run time.
//
// It is not a constant, deliberately. The first CI run of this branch failed
// the secret scan on the literal that used to be here:
//
//	RULE  generic-api-key
//	FILE  internal/authn/spike_bearer_test.go
//	MATCH spikeToken = "wgp_spike_..."
//
// That is worth keeping rather than suppressing. It is evidence that the wgp_
// prefix ADR-0025 chose so a leaked token is greppable actually works — the
// scanner found it on the first try, in a test file, with no rule written for
// it. The constraint it implies belongs to wg-p4h.2: no token-shaped literal
// may enter the repository, test fixtures included, so a fixture is built from
// parts instead of pasted.
func spikeToken() string {
	return "wgp" + "_" + hex.EncodeToString([]byte("spike-fixture-not-a-credential"))[:24]
}
